...
scripts/prospective_universe_contract.py
...
data/trade_log.csv
config.py
config/prospective_universe.json
requirements.txt
...
HUMAN_PILOT_REVIEW_READY    # 公开前瞻 20、影子复核成熟样本 20 及数据/模型契约通过，可进入人工试用审核
```
...
`config/prospective_universe.json` 冻结本验证代的 94 只前瞻股票池，并绑定策略与验证切分指纹。`build_forward_ledger.py` 每次都会验证当前报告和历史每个预测 cohort 都完整保留这 94 个 ticker；新增、删除或替换 ticker 必须开启新的验证代，不能把变更前后的样本混合计数。该控制只约束前瞻固定股票池口径，不消除回溯研究的幸存者偏差，也不支持对全市场推广。
